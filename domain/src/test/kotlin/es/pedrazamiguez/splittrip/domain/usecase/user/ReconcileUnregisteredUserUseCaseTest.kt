...
class ReconcileUnregisteredUserUseCaseTest {
    ...
    fun setUp() {
        ...
        coEvery { userPreferenceRepository.getIsReconciled() } returns flowOf(false)
        coEvery { userPreferenceRepository.setIsReconciled(any()) } returns Unit
    }
    ...
    inner class Invoke {
        ...
        @Test
        fun `reconciles user, deletes pending user, and sets reconciled to true when successful`() = runTest {
            // Given
        ...
        fun `returns early and does nothing if already reconciled`() = runTest {
            ...
            val email = "pending@example.com"
            val activeUserId = "active-user-123"
            coEvery { userPreferenceRepository.getIsReconciled() } returns flowOf(true)
            ...
            // Then
            assertTrue(result.isSuccess)
            coVerify(exactly = 0) { groupRepository.reconcileUnregisteredUser(any(), any()) }
            coVerify(exactly = 0) { userRepository.deletePendingUser(any()) }
            coVerify(exactly = 0) { userPreferenceRepository.setIsReconciled(any()) }
        }

        @Test
        fun `returns failure and does not set reconciled when group reconciliation fails`() = runTest {
            // Given
            val email = "pending@example.com"
            val activeUserId = "active-user-123"
            val pendingUserId = User.generatePendingUserId(email)
        ...
        @Test
        fun `returns failure and does not set reconciled when deleting pending user fails`() = runTest {
            // Given
            val email = "pending@example.com"
            val activeUserId = "active-user-123"
            val pendingUserId = User.generatePendingUserId(email)
