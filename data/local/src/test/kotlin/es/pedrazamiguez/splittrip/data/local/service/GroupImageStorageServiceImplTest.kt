...
class GroupImageStorageServiceImplTest {
    ...
    @Test
    fun cleanTempGroupImages_removesOldCameraTempFiles() = runTest {
        // Given
        val tempDir = File(context.filesDir, "groups_temp").also { it.mkdirs() }
        val oldCameraFile = File(tempDir, "group_camera_old.jpg").also { it.writeText("old") }
        val newCameraFile = File(tempDir, "group_camera_new.jpg").also { it.writeText("new") }

        // Set modification times
        val thresholdTime = System.currentTimeMillis() - (15 * 60 * 1000L) // 15 mins ago
        oldCameraFile.setLastModified(thresholdTime)
        newCameraFile.setLastModified(System.currentTimeMillis())

        // When
        service.cleanTempGroupImages()

        // Then
        assertTrue(!oldCameraFile.exists())
        assertTrue(newCameraFile.exists())
    }

    @Test
    fun saveTempGroupImage_recyclesRotatedAndDecodedBitmapOnException() = runTest {
        // Given
        val sourceUriStr = "content://media/picker/image"
        val mockUri = mockk<Uri>(relaxed = true)
        ...
        // Throw exception on compress to trigger catch block
        every { mockOriginalBitmap.compress(any(), 80, any()) } throws RuntimeException("Disk full")

        // When
        try {
            service.saveTempGroupImage(sourceUriStr)
            org.junit.Assert.fail("Expected RuntimeException")
        } catch (e: RuntimeException) {
            assertEquals("Disk full", e.message)
        }

        // Then — verify original bitmap recycled
        verify(exactly = 1) { mockOriginalBitmap.recycle() }
    }

    @Test
    fun commitGroupImage_resolvesContentUriAndCopies() = runTest {
        // Given
        val groupId = "group-456"
        val tempDir = File(context.filesDir, "groups_temp").also { it.mkdirs() }
        val tempFile = File(tempDir, "group_temp_content.webp").also { it.writeText("Content Uri Temp Bytes") }

        val contentUriStr = "content://es.pedrazamiguez.splittrip.fileprovider/groups_temp/group_temp_content.webp"
        val mockUri = mockk<Uri>(relaxed = true)
        ...
        every { mockUri.pathSegments } returns listOf("groups_temp", "group_temp_content.webp")

        every { context.packageName } returns "es.pedrazamiguez.splittrip"

        // When
        val resultUri = service.commitGroupImage(groupId, contentUriStr)
        ...
        val destFile = File(permanentDir, "$groupId.webp")
        assertTrue(destFile.exists())
        assertEquals("Content Uri Temp Bytes", destFile.readText())
        assertTrue(!tempFile.exists())
    }

    @Test
    fun deleteLocalGroupImage_fileDoesNotExist_noop() = runTest {
        // When/Then - should complete without throwing exception
        service.deleteLocalGroupImage("non-existent-group")
    }

    @Test
    fun cleanTempGroupImages_tempDirDoesNotExist_noop() = runTest {
        // Given - delete the temp dir
        val tempDir = File(context.filesDir, "groups_temp")
        if (tempDir.exists()) {
    ...
    @Test
    fun saveTempGroupImage_appliesRotation() = runTest {
        // Given
        val sourceUriStr = "content://media/picker/image"
        val mockUri = mockk<Uri>(relaxed = true)
        ...
        every {
            anyConstructed<androidx.exifinterface.media.ExifInterface>().getAttributeInt(
                androidx.exifinterface.media.ExifInterface.TAG_ORIENTATION,
                androidx.exifinterface.media.ExifInterface.ORIENTATION_NORMAL
            )
        } returns androidx.exifinterface.media.ExifInterface.ORIENTATION_ROTATE_90

        // Mock bitmap rotation
        ...
        every { mockRotatedBitmap.compress(any(), 80, any()) } answers {
            val outStream = arg<java.io.OutputStream>(2)
            outStream.write("Compressed Rotated Bytes".toByteArray())
            true
        }
        ...
        val tempDir = File(context.filesDir, "groups_temp")
        val savedFiles = tempDir.listFiles() ?: emptyArray()
        assertTrue(savedFiles.any { it.readText() == "Compressed Rotated Bytes" })

        unmockkConstructor(androidx.exifinterface.media.ExifInterface::class)
    }

    @Test
    fun saveTempGroupImage_deletesSourceCameraFileInFinally() = runTest {
        // Given
        val sourceUriStr = "content://es.pedrazamiguez.splittrip.fileprovider/groups_temp/group_camera_123.jpg"
        val mockUri = mockk<Uri>(relaxed = true)

        mockkStatic(Uri::class)
        every { Uri.parse(sourceUriStr) } returns mockUri
        every { mockUri.authority } returns "es.pedrazamiguez.splittrip.fileprovider"
        every { mockUri.lastPathSegment } returns "group_camera_123.jpg"
        every { context.packageName } returns "es.pedrazamiguez.splittrip"

        val tempDir = File(context.filesDir, "groups_temp").also { it.mkdirs() }
        val sourceFile = File(tempDir, "group_camera_123.jpg").also { it.writeText("Raw Jpeg Bytes") }
        ...
        // Then
        assertNotNull(resultUri)
        // Verify the source raw file was deleted
        assertFalse(sourceFile.exists())
    }

    @Test
    fun saveTempGroupImage_largeDimensions_calculatesInSampleSize() = runTest {
        // Given
        val sourceUriStr = "content://media/picker/image"
        val mockUri = mockk<Uri>(relaxed = true)
        ...
        every { mockResolver.openInputStream(mockUri) } returns ByteArrayInputStream("Dummy".toByteArray())
        ...
        val mockOriginalBitmap = mockk<Bitmap>(relaxed = true)
        every { mockOriginalBitmap.width } returns 1250
        every { mockOriginalBitmap.height } returns 750
        every { mockOriginalBitmap.isRecycled } returns false
        every { BitmapFactory.decodeStream(any(), null, match { !it.inJustDecodeBounds }) } returns mockOriginalBitmap
    ...
    @Test
    fun saveTempGroupImage_resolverThrows_fallsBackToLocalFile() = runTest {
        // Given
        val sourceUriStr = "content://es.pedrazamiguez.splittrip.fileprovider/groups_temp/group_camera_123.jpg"
        val mockUri = mockk<Uri>(relaxed = true)
        ...
        every { mockUri.scheme } returns "content"
        every { mockUri.authority } returns "es.pedrazamiguez.splittrip.fileprovider"
        every { mockUri.pathSegments } returns listOf("groups_temp", "group_camera_123.jpg")
        every { context.packageName } returns "es.pedrazamiguez.splittrip"

        val tempDir = File(context.filesDir, "groups_temp").also { it.mkdirs() }
        val sourceFile = File(tempDir, "group_camera_123.jpg").also { it.writeText("Raw Local File Content") }

        // Throw on contentResolver.openInputStream
        every { mockResolver.openInputStream(mockUri) } throws RuntimeException("Permission denied")
    ...
    @Test
    fun saveTempGroupImage_cannotOpenInputStream_throwsException() = runTest {
        // Given
        val sourceUriStr = "content://media/picker/image"
        val mockUri = mockk<Uri>(relaxed = true)
        ...
        try {
            ...
            assertTrue(e.message?.contains("Could not open input stream") == true)
        }
    }

    @Test
    fun saveTempGroupImage_decodeOriginalBitmapReturnsNull_throwsException() = runTest {
        // Given
        val sourceUriStr = "content://media/picker/image"
        val mockUri = mockk<Uri>(relaxed = true)
        ...
        try {
            ...
            assertTrue(e.message?.contains("Could not decode image") == true)
        }
    }
}
